...
class PracticeMatching::PracticeInterest < ActiveRecord::Base
  ...
  validates :user_id,
            uniqueness: {
              ...
              message: I18n.t("practice_matching.errors.already_exists")
            }
  ...
  def cannot_add_self
    if user_id == target_user_id
      errors.add(
        :target_user_id,
        I18n.t("practice_matching.errors.cannot_add_self")
      )
    end
  end
  ...
  def check_for_mutual_match
    # 检查是否形成双向匹配
    if PracticeMatching::PracticeInterest.mutual_match_exists?(
         user_id,
         target_user_id
       )
      notify_mutual_match
    ...
    [user, target_user].each do |matched_user|
      ...
      Notification.create!(
        ...
        data: {
          ...
          topic_title:
            I18n.t(
              "practice_matching.messages.mutual_match_notification",
              username: other_user.username,
              locale: matched_user.effective_locale
            ),
          # 添加更多上下文信息
